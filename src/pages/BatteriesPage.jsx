...
import {
  ...
  getBatteryCategoryLabel,
  normalizeBatteryTypeSlug,
} from '../data/batteryCategories'
import { getMergedBatteriesPage } from '../site/siteStore'

export default function BatteriesPage() {
  useSiteUpdate()
  const [searchParams] = useSearchParams()
  const typeSlug = normalizeBatteryTypeSlug(searchParams.get('type'))
  const { products, heroSlides, meta } = getMergedBatteriesPage()

  const filteredProducts = useMemo(
    () => filterBatteriesByType(products, typeSlug),
    [products, typeSlug],
  )

  const sectionTitle =
    typeSlug === 'all' ? meta.title : getBatteryCategoryLabel(typeSlug)
  ...
  return (
    <div className="bg-app-bg">
      ...
      >
        <SectionHeading eyebrow="Batteries" title={sectionTitle} />
        <p className="-mt-4 mb-10 max-w-2xl text-sm leading-relaxed text-exclusive-muted md:text-base">
          {typeSlug === 'all'
            ? meta.blurb
            : `Showing ${getBatteryCategoryLabel(typeSlug).toLowerCase()} from our catalog.`}
        </p>
        {products.length === 0 ? (
          ...
          <p className="rounded-md border border-dashed border-app-border-strong py-16 text-center text-exclusive-muted">
            No products for <span className="font-medium text-exclusive-dark">{sectionTitle}</span> yet.
            Pick another type in the Batteries menu or assign a battery type in admin.
          </p>
        ) : (
